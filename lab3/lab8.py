...
def segment_image(image, segment_size):
    ...
    return segments, (h + segment_size - 1) // segment_size, (w + segment_size - 1) // segment_size
...
def full_restore(image, segments, rows, cols, segment_size, method="rows"):
    ...
    for segment, i, j in tqdm(segments, total=len(segments), desc=f"Restoring Image ({method})"):
        ...
        restored_image[i:i + segment.shape[0], j:j + segment.shape[1]] = np.maximum(
            restored_image[i:i + segment.shape[0], j:j + segment.shape[1]],
            np.clip(restored_segment, 0, 255)
        )
...
def approximate_rows(segment, keep_coeff=4):
    """
    Відновлення сегмента за допомогою ДКП по рядках.
    :param segment: Вхідний сегмент зображення.
    :param keep_coeff: Кількість коефіцієнтів, які залишаються для відновлення.
    :return: Відновлений сегмент і список помилок СКВ для кожного рядка.
    """
    restored_segment = np.zeros_like(segment, dtype=float)
    mse_list = []
    for row in range(segment.shape[0]):
        dct_row = dct(segment[row, :], norm='ortho')  # ДКП для рядка
        dct_row[keep_coeff:] = 0  # Залишаємо лише перші `keep_coeff` коефіцієнтів
        restored_row = idct(dct_row, norm='ortho')  # Обернене ДКП
        restored_segment[row, :] = np.clip(restored_row, 0, 255)
        mse_list.append(mean_squared_error(segment[row, :], restored_row))
    return restored_segment, mse_list


# Функція для апроксимації сегментів по стовпцях
def approximate_columns(segment, keep_coeff=4):
    """
    Відновлення сегмента за допомогою ДКП по стовпцях.
    :param segment: Вхідний сегмент зображення.
    :param keep_coeff: Кількість коефіцієнтів, які залишаються для відновлення.
    :return: Відновлений сегмент і список помилок СКВ для кожного стовпця.
    """
    restored_segment = np.zeros_like(segment, dtype=float)
    mse_list = []
    for col in range(segment.shape[1]):
        dct_col = dct(segment[:, col], norm='ortho')  # ДКП для стовпця
        dct_col[keep_coeff:] = 0  # Залишаємо лише перші `keep_coeff` коефіцієнтів
        restored_col = idct(dct_col, norm='ortho')  # Обернене ДКП
        restored_segment[:, col] = np.clip(restored_col, 0, 255)
        mse_list.append(mean_squared_error(segment[:, col], restored_col))
    return restored_segment, mse_list


# Функція для створення маски видалених зон
def create_removed_zones_mask(image, image_shape, segments):
    mask = np.copy(image)  # Спочатку копіюємо оригінальне зображення
    for _, i, j in segments:
        mask[i:i + segment_size, j:j + segment_size] = 233
    return mask
...
def visualize_row_regression(segment, restored_segment, entropy_label):
    ...
    for row in range(num_rows):
        ax = axes[row]
        ax.plot(x, segment[row, :], label='Original Row', color='blue')
        ax.plot(x, restored_segment[row, :], label='Restored Row', linestyle='--', color='orange')
        ax.set_title(f'{entropy_label} Entropy: Row {row + 1}', fontsize=10)
        ax.set_xlabel('Pixel Position', fontsize=9)
...
def visualize_column_regression(segment, restored_segment, entropy_label):
    ...
    segment = np.clip(segment, 0, 255).astype(np.uint8)
    restored_segment = np.clip(restored_segment, 0, 255).astype(np.uint8)

    # print('Original Segment:', segment)
    # print('Restored Segment:', restored_segment)

    for col in range(num_cols):
        ax = axes[col]
        ax.plot(x, segment[:, col], label='Original Column', color='blue')
        ax.plot(x, restored_segment[:, col], label='Restored Column', linestyle='--', color='orange')
        ax.set_title(f'{entropy_label} Entropy: Column {col + 1}', fontsize=10)
        ax.set_xlabel('Pixel Position', fontsize=9)
        ax.set_ylabel('Intensity', fontsize=9)
        ax.legend(fontsize=8)
        ax.grid(alpha=0.5)
    ...
    ax_original.set_title(f'{entropy_label} Entropy: Original Segment', fontsize=10)
...
def main(image_path, segment_size):
    ...
    brightness_channel = load_image(image_path)
    ...
    # Повне відновлення зображення за рядками та стовпцями
    ...
    # print(f"СКВ для повністю відновленого зображення за рядками: {snr_rows:.4f}")
    # print(f"СКВ для повністю відновленого зображення за колонками: {snr_columns:.4f}")
    ...
    low_entropy_mask = create_removed_zones_mask(brightness_channel, brightness_channel.shape, low_entropy_segments)
    medium_entropy_mask = create_removed_zones_mask(brightness_channel, brightness_channel.shape, medium_entropy_segments)
    high_entropy_mask = create_removed_zones_mask(brightness_channel, brightness_channel.shape, high_entropy_segments)
    ...
    # Виведення статистики для СКВ (Signal-to-Noise Ratio) для всього зображення
    def calculate_snr(original_image, restored_image):
        """Calculate the SNR for the entire image."""
        noise = original_image - restored_image
        snr = np.mean(original_image) / np.std(noise)
        return snr

    # Розрахунок SNR для відновлених зображень
    snr_rows = calculate_snr(brightness_channel, fully_restored_rows)
    snr_columns = calculate_snr(brightness_channel, fully_restored_columns)
    ...
    # Оригінальне зображення
    plt.subplot(1, 3, 1)
    print(f"Min intensity: {np.min(brightness_channel)}, Max intensity: {np.max(brightness_channel)}")
